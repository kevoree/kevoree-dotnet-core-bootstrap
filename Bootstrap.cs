            ...
            if (Parser.Default.ParseArguments(args, options))
            {
                try
                {
                    ...
                    if (options.ScriptPath == null)
                    {
                         ...
                         * The Dot	net Node name is "nodeName".
                         * The WSGroup listen on port 9000.
                         */
                        var defaultKevScript = string.Format(@"add {0} : dotnet.dotnetNode/1/LATEST 
...
attach {0} sync", options.NodeName);
        ...
        private void LoadKevScript(string defaultKevScript, UpdateCallback callback)
        {
			...
			var jsonSer = Core.getFactory().createJSONSerializer();

			Console.WriteLine(jsonSer.serialize(emptyModel));
